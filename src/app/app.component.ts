...
export class AppComponent {
  ...
  ngOnInit() {
    this.http.get("https://testologia.ru/cookies").subscribe(
      data => {
  ...
  scrollTo(target: HTMLElement, cookie?: Cookie) {
    target.scrollIntoView({behavior: "smooth"});
    if (cookie) {
      this.form.patchValue({
        product: `${cookie.title} (${cookie.price} ${this.currency})`,
      });
    }
  }
  ...
  changeCurrency() {
    ...
    this.productsData.forEach((product: any) => {
        product.price = (product.basePrice * coefficient).toFixed(0);
    });
  }
